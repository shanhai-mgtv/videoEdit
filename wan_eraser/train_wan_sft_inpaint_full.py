...
def prepare_mask_latent_size(mask_values: torch.Tensor, nframes: int) -> torch.Tensor:
    ...
    Returns:
        mask_lat_size: [B, 1, F//4, H//16, W//16] mask at latent size
    """
...
def main(cfg: Config):
    ...
    accelerator.print(f">> Training Mode: Full-Parameter Fine-tuning")
    ...
    with torch.no_grad():
        logger.info("Checking transformer input channels for three-branch temporal+channel fusion")
        initial_input_channels = transformer.config.in_channels
        print(f"Initial input channels: {initial_input_channels}")
    ...
    logger.info("All transformer parameters will be trained")
    logger.info("=" * 60)
    
    # Enable gradients for all transformer parameters
    ...
    # ======================================================
    from dataloading.dataset_inpaint_with_mask_video import MaskVideoInpaintingDataset
    
    # Enable debug saving for first few samples
    debug_save_dir = output_dirpath / "debug_data"
    
    train_dataset = MaskVideoInpaintingDataset(
        cfg.data,
        save_debug=True,
    ...
    # =======================================================
    # 4. distributed training preparation with acceleratorz
    # =======================================================
    ...
    logger.info(f"  Training Mode .................................................. Full-Parameter Fine-tuning")
    ...
    def save_training_visualization(batch, global_step, save_dir):
        ...
        target = batch["target_images"][0].cpu().numpy()
        masked = batch["masked_video"][0].cpu().numpy()
        mask_v = batch["mask_video"][0].cpu().numpy()
        ref = batch["ref_image"][0].cpu().numpy()
        mask_i = batch["mask_image"][0].cpu().numpy()
        ref_masked_i = batch["ref_masked_image"][0].cpu().numpy()
        ref_frame_idx = batch.get("ref_frame_idx", [0])[0]
        ...
        # Save info
        with open(vis_dir / "info.txt", "w") as f:
            f.write(f"caption: {caption}\n")
            f.write(f"ref_frame_idx: {ref_frame_idx}\n")
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                with torch.no_grad():
                    ...
                    mask_img_lat_size = F.interpolate(mask_img, scale_factor=1/16, mode="nearest-exact")
                ...
                zeros_latents_4ch = torch.zeros_like(mask_lat_size)
                ...
                # # - ref背景token被屏蔽，只关注前景
                # # ============================================
                # p_h, p_w = 2, 2  # patch size
                
                # # ref_token_mask: [B, num_ref_tokens], 1=foreground(关注), 0=background(屏蔽)
                ...
                # ref_token_mask = (ref_token_mask > 0.5).float()  # 1=foreground, 0=background
                ...
                # Compute per-sample loss then reduce to scalar
                ...
                loss = per_sample_loss.mean()  # Final reduction to scalar for backward

                # 简单的本地NaN检查（不使用集合操作，避免死锁）
                if torch.isnan(loss) or torch.isinf(loss):
                    logger.warning(f"[Rank {accelerator.process_index}] NaN/Inf loss={loss.item()}, step={global_step}")
                    # 不跳过，让backward正常执行，DeepSpeed会处理
            ...
            if accelerator.sync_gradients:
                ...
                # Checkpoint saving: all ranks must participate for distributed sync
