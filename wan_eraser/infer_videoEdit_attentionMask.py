        ...
        with torch.no_grad():
            with torch.autocast("cuda", dtype=load_dtype):
                ...
                with torch.inference_mode():
                    ...
                    # 准备 ref_mask
                    # 这里的 ref_mask_tensor 已经是 [1, 1, infer_h, infer_w] 且值在 [0, 1]
                    ...
                    # 缩放到 latent 尺寸 (匹配训练逻辑: 原始分辨率 -> 1/16)
                    # 训练代码: mask_img_lat_size = F.interpolate(mask_img, scale_factor=1/16, mode="nearest-exact")
                    mask_img_lat_size = F.interpolate(ref_mask_tensor, size=(mask_lat_size.shape[3], mask_lat_size.shape[4]), mode="nearest-exact")
                    mask_img_lat_size = mask_img_lat_size.unsqueeze(2).repeat(1, 4, 1, 1, 1)  # [1, 4, 1, h, w]
                    
                    # ============================================
                    # Prepare attention masks (same as training)
                    # ============================================
                    p_t, p_h, p_w = 1, 2, 2  # patch size
                    ...
                    # # video_token_mask: [B, num_video_tokens], 1=visible, 0=masked
                    # video_mask_latent = mask_lat_size[:, 0:1, :, :, :].squeeze(1)  # [B, F, H_lat, W_lat]
                    # B_v, F_lat, H_lat, W_lat = video_mask_latent.shape
                    # video_mask_latent = video_mask_latent.reshape(B_v * F_lat, 1, H_lat, W_lat)
                    # video_mask_patches = F.avg_pool2d(video_mask_latent, kernel_size=(p_h, p_w), stride=(p_h, p_w))
                    # video_mask_patches = video_mask_patches.reshape(B_v, -1)  # [B, num_video_tokens]
                    # # Invert: visible region (mask=0) -> 1, masked region (mask=1) -> 0
                    # video_token_mask = (video_mask_patches < 0.5).float()
                    
                    print(f"[DEBUG] ref_token_mask: shape={ref_token_mask.shape}, foreground ratio={ref_token_mask.mean():.2%}")
                    print("=" * 60)
...
def main():
    ...
    parser.add_argument("--output_path", type=str, default="/mnt/shanhai-ai/shanhai-workspace/lihaoran/project/code/videoEdit/videoEdit/wan_eraser/validation_demo/sample_0012/1229_first_norope_attentionMask_200_vae.mp4", help="Output video path")
    ...
    parser.add_argument(
        ...
        type=str,
        # default = None,
        default="/mnt/shanhai-ai/shanhai-workspace/lihaoran/project/code/videoEdit/videoEdit/wan_eraser/outputs/lora_1229_first_norope/inpaint_lora_toqkv/checkpoint-step00000200/lora_adapter",
    ...
    parser.add_argument("--infer_h", type=int, default=480, help="Inference height (must match training)")
    parser.add_argument("--infer_w", type=int, default=640, help="Inference width (must match training)")
