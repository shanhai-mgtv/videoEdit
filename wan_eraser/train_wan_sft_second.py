"""
Video Inpainting Training Script with Three-Branch Temporal + Channel Fusion
...
Based on train_wan_sft.py for Wan 2.2 I2V model.
"""
...
def main(cfg: Config):
    ...
    transformer = WanTransformer3DModel.from_pretrained(
        "/mnt/shanhai-ai/shanhai-workspace/lihaoran/project/code/videoEdit/videoEdit/wan_eraser/outputs/lora_1225_second_ch148_patchembedding_merge",
        subfolder="transformer_148",
        torch_dtype=load_dtype,
    )

    # with torch.no_grad():
    #     initial_input_channels = 100  # 原始输入通道数（如16）
    #     new_patch_embedding = nn.Conv3d(
    #             in_channels=148,  # 新输入通道数（原通道数×2）
    #             out_channels=transformer.config.num_attention_heads * transformer.config.attention_head_dim,
    ...
    #     save_path = '/mnt/shanhai-ai/shanhai-workspace/lihaoran/project/code/videoEdit/videoEdit/wan_eraser/outputs/lora_1225_second_ch148_patchembedding_merge/transformer_148'   # 自定义保存路径（避免覆盖原模型）
    ...
    def save_model_hook(models, weights, output_dir):
        ...
        if cfg.hparams.ema.use_ema and ema_model is not None:
            ...
            from pipelines.pipeline_wan_inpainting import WanPipeline
    ...
    for epoch in range(first_epoch, cfg.hparams.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            # ============================================
            # Save visualization for first few steps
            # ============================================
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                with torch.no_grad():
                    ...
                    mask_video_latents = prepare_mask_latent_size(mask_video, cfg.data.nframes) # 4通道 -> [B 4 f h w]
                    num_latent_frames = mask_video_latents.shape[2]
                    
                    # ref_mask 尺度 -> [B, 4, 1, h, w], mask_video -> [B, 4, F, h, w]
                    # ref_latents 尺度 -> [B, 16, 1, h, w]
                    ref_mask_latents = F.interpolate(mask_img, scale_factor=1/16, mode="nearest-exact")  # [B, 1, h, w]
                    ref_mask_latents = ref_mask_latents.unsqueeze(2).repeat(1, 4, 1, 1, 1)  # [B, 4, 1, h, w]
                ...
                zeros_latents_4ch = torch.zeros_like(mask_video_latents)  # [B, 4, F, h, w]

                branch1 = torch.cat ([noisy_model_input, ref_latents], dim = 2) # [b 48 22 h w ]
                branch2 = torch.cat ([masked_video_latents, ref_latents], dim = 2) # [b 48 22 h w ]
                branch3 = torch.cat ([mask_video_latents, ref_mask_latents], dim = 2) # [b 4 22 h w ]
                branch4 = torch.cat ([zeros_latents, ref_latents], dim = 2) # [b 48 22 h w ]
                model_input = torch.cat([branch1, branch2, branch3, branch4], dim=1)
                
                
                # Debug shape verification (first step only)
                if step == 0 and accelerator.is_main_process:
                    logger.info(f"Branch shapes - branch1: {branch1.shape}, branch2: {branch2.shape}, branch3: {branch3.shape}, branch4: {branch4.shape}")
                ...
                weight_mask = mask_video_latents[:, 0:1]  # [B, 1, F, H, W]
